# This site and Nuvenhub

This file is installed by `nuvenhub-site-kit`. It describes every connection between this
website and the Nuvenhub platform, so nothing about the relationship is hidden in code.

**This site does not depend on Nuvenhub.** It is hosted on your own Cloudflare account, its
code is in your own repository, and (for a shop) its database is your own D1. If you stop
using Nuvenhub, the site keeps working and stays fully editable. Nothing here is a lock-in.
...
One block in `index.html`, between `<!-- nuvenhub:tracking:start -->` and
`<!-- nuvenhub:tracking:end -->`.

It loads a small script from Nuvenhub that counts page views, including client-side route
...
It is switched on by a **build variable**, `VITE_NUVENHUB_TRACKING_SRC`, set in Cloudflare
...
Nuvenhub manages your products and orders by calling this site's own `/api/admin/*`
endpoints with a token stored in your `admin_tokens` table (as a hash — the raw token is
not recoverable from the database).

**Nuvenhub has no private back door.** It calls exactly the same endpoints your own `/admin`
...
## Running the shop without Nuvenhub — shop sites only

Your own admin lives at **`/admin`** on this site, and it does not talk to Nuvenhub at all.
